...
func _spawn_rooms() -> void:
	...
	for i in num_levels:
		...
		if i == 0:
			...
			var exit_tile_pos: Vector2 = (previous_room_tilemap.local_to_map(previous_room_door.position) + Vector2i.UP) * 0.2
			
			var corridor_height: int = randi() % 5 + 4
			...
			var entrance_right_pos: Vector2 = room_tilemap.local_to_map(room.get_node("Entrance/RightTile").position) * 0.2
#			
			room.position.y = ((previous_room_door.global_position.y)
			- (room_tilemap.get_used_rect().size.y * TILE_SIZE)
			- ((corridor_height - 2) * TILE_SIZE))
			room.position.x = exit_tile_pos.x * TILE_SIZE
